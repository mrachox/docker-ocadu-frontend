...
import igniteCurrentEvents from '@/components/events/IgniteCurrentEvents'
import ignitePastEvents from '@/components/events/IgnitePastEvents'
import contactIgnite from '@/components/contact/ContactIgnite'
...
export default {
  ...
  methods: {
    getAllPosts() {
      ...
      axios.get(this.ep)
        .then(response => {
          this.posts = response.data.filter(post => post.nid === "100056575")
