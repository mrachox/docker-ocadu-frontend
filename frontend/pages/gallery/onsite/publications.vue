<template>
<div id="gallerypub" class="onsite">
  <galleriesSlugSubNav />
  <div v-for="post in posts" v-bind:key="post.nid">
    <Hero class="onsite-logo" :variableImg="'https://ocadu.ca/' + post.field_ip_header_image" :variableText="post.title" :variableSubhead="post.field_subtitle" :variableCredit="post.field_credit" />
...
export default {
  ...
  methods: {
    getAllPosts() {
      ...
      axios.get(this.ep)
        .then(response => {
          this.posts = response.data.filter(post => post.nid === "100066008")
