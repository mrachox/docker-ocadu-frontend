<template>
  <div>
    ...
    <undergradPrograms :faculty="fod" ugDegree="in Design (BDes)" :showDesc="true" :showCTA="true"/>
  </div>
</template>
...
export default {
  ...
  methods: {
    getAllPosts() {
      ...
      axios.get(this.ep)
        .then(response => {
          this.posts = response.data.filter(post => post.nid === '100065117')
