...
<div class="child--academicprogram mb-5">
  <div v-for="post in posts" v-bind:key="post.field_slug">
    ...
    <b-container>
      <!-- TODO MISSING GRAD PROGRAM NEWS -->
      <div class="page-nav text-center my-4 text-uppercase" v-html="post.field_quicklinks"></div>
      <div class="grid-row the-content one-col" v-html="post.body"></div>
      <programGuides />
    </b-container>
  </div>
</div>
    ...
    getAllPosts() {
      ...
      axios.get(this.ep)
        .then(response => {
          ...
          this.posts = response.data.filter(post => post.nid === '100054954')
