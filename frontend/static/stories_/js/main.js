...
domready(function() {
    ...
    header = document.querySelector('.sidebar'),
    ...
    close.style.top = open.offsetTop + "px";

		menu.classList.add('active');
		eventie.bind(document, 'touchmove', disableScroll);
		eventie.bind(menuItems, 'touchmove', menuScroll);

    eventie.unbind(document, 'touchmove', onScrollMove);
    eventie.unbind(document, 'scroll', onScrollMove);
	}
    ...
    eventie.bind(document, 'touchmove', onScrollMove);
    eventie.bind(document, 'scroll', onScrollMove);
	}
  ...
  if(isMobile()) {
    ...
    wrap.style.paddingTop = header.offsetHeight + 10 + "px";

    document.addEventListener('click', function(event) {
      if(document.querySelector('.menu').classList.contains("active") && event.target != open) {
        hideMenu(event);
      }
    });
    ...
    using.addEventListener('click', function(event) {
      event.stopPropagation();
    });
  }
  ...
  if(isMobile() || isTablet()) {
    blogVideoHeight();
    eventie.bind(document, 'resize', blogVideoHeight);
  }
