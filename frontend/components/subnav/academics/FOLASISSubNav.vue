<template>
  <div class="component--folasis">
    <div id="top">
        <ul class="sub-nav">
          <!--CHILDPAGES-->
          <li v-for="child_pages_post in child_pages_posts" v-bind:key="child_pages_post.nid">
            <nuxt-link :to="'/academics/faculty-of-arts-and-science/' + child_pages_post.field_slug">{{ child_pages_post.title }}</nuxt-link>
          </li>
          <!--PROGRAMS-->
...
export default {
  data () {
    return {
      child_pages_posts: '',
      posts: '',
      epChP: 'https://www.ocadu.ca/api/folasis-child-pages',
  ...
  methods: {
    ...
    getAllPosts() {
      axios.get(this.ep)
        .then(response => {
          this.posts = response.data.filter(post => post.field_ip_department.includes('Liberal'))
        })
        .catch(error => {
          console.log('-----error------- ' + slug);
          console.log(error);
