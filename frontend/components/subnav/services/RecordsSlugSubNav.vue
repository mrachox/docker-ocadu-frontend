<template>
  <div class="component--slugsubnav">
    <div id="top">
        <ul class="sub-nav">
          <li v-for="post in posts" v-bind:key="post.nid">
            <nuxt-link :to="'/services/records-and-registration/' + post.field_slug">{{ post.title }}</nuxt-link>
          </li>
        </ul>
    </div>
  </div>
...
export default {
  data () {
    return {
      posts: null,
      epIT: 'https://www.ocadu.ca/api/records-pages',
    }
  },
  mounted () {
