<template>
  <div class="my-5">
    <h2 class="text-center my-4">Undergraduate Programs {{ugDegree}}</h2>
    <b-container>
      <b-row>
        <b-col sm="12" md="6" lg="3" v-for="post in filteredUndergradPrograms" v-bind:key="post.id">
...
export default {
  props: ['faculty', 'ugDegree', 'showDesc', 'showCTA'],
  data () {
    return {
      filteredFaculty: this.faculty,
      posts: '',
      filteredUndergradPrograms: [],
    ...
    getAllPosts() {
      ...
      axios.get(this.ep)
        .then(response => {
          ...
          this.filteredUndergradPrograms = this.posts.filter(post => post.field_ip_department.includes(this.filteredFaculty))
        })
        .catch(error => {
