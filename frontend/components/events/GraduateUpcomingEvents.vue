<template>
  <div class="my-4">
    <h2 class="my-5 text-center">Upcoming &amp; Current Events</h2>
...
export default {
  data() {
    return {
      posts: null,
      ep: 'https://www.ocadu.ca/api/upcoming-graduate-events',
    }
  },
  created() {
    this.getAllPosts();
    ...
    getAllPosts() {
      ...
      axios.get(this.ep)
        .then(response => {
          ...
          this.posts = response.data.filter(post => post.field_type.includes('Events'))
        })
        .catch(error => {
